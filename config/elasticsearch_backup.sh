...
elasticdump --input=http://${ES_USERNAME}:${ES_PASSWORD}@localhost:9200/  --output=/backups/data.json --type=data --limit=5000
ssh -oStrictHostKeyChecking=no -o ExitOnForwardFailure=yes -f -L 9200:localhost:${ES_PORT} ${SSH_USERNAME}@${SSH_HOST} -p ${SSH_PORT} sleep 10
elasticdump --input=http://${ES_USERNAME}:${ES_PASSWORD}@localhost:9200/  --output=/backups/mapping.json --type=mapping --limit=5000
